  ...
  return (
    <div >
      <div className='flex items-center flex-col'>
        ...
        <p className='font-bold'>{saleInvoice?.order?.branch?.name}</p>
        <p>Địa chỉ: <span>{saleInvoice?.order?.branch?.ward?.name} - {saleInvoice?.order?.branch?.district?.name} - {saleInvoice?.order?.branch?.province?.name}</span></p>
        <p>
          SĐT: <span>{saleInvoice?.order?.branch?.phone}</span>
        </p>
        <h4 className='text-lg font-bold'>HÓA ĐƠN BÁN HÀNG</h4>
        <p className='font-bold'>Số HĐ: <span>{saleInvoice?.order?.code}</span></p>
        <p>Ngày tạo: <span>{formatDateTime(saleInvoice?.order?.createdAt)}</span></p>
      </div>
      <div className='mt-5'>
        <p>Khách hàng: <span className='ml-1'>{saleInvoice?.order?.customer?.fullName}</span></p>
        <p>SĐT: <span className='ml-1'>{saleInvoice?.order?.customer?.phone}</span></p>
        <p>Địa chỉ: <span className='ml-1'>{saleInvoice?.order?.customer?.address}</span></p>
      </div>
