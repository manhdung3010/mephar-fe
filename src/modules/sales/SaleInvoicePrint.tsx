    ...
    <div>
      <div className="flex items-center flex-col">
        ...
        <p className="font-bold">
          {saleInvoice?.order?.branch?.name ?? saleInvoice?.branch?.name}
        </p>
        <p>
          Địa chỉ:
          {saleInvoice?.order ? (
            <span>
              {saleInvoice?.order?.branch?.ward?.name} -{" "}
              {saleInvoice?.order?.branch?.district?.name} -{" "}
        ...
        <p>
          SĐT:{" "}
          <span>
            {saleInvoice?.order?.branch?.phone ?? saleInvoice?.branch?.phone}
          </span>
        </p>
        ...
        <p>
          ...
          <span>
            {formatDateTime(
              saleInvoice?.order?.createdAt ?? saleInvoice?.createdAt
            )}
          </span>
        </p>
      </div>
      <div className="mt-5">
        <p>
          Khách hàng:{" "}
          <span className="ml-1">
            {saleInvoice?.order?.customer?.fullName ??
              saleInvoice?.customer?.fullName}
          </span>
        </p>
        <p>
          SĐT:{" "}
          <span className="ml-1">
            {saleInvoice?.order?.customer?.phone ??
        ...
        <p>
          Địa chỉ:{" "}
          <span className="ml-1">
            {saleInvoice?.order?.customer?.address ??
      ...
      <div className="overflow-x-auto my-4">
        <table className="table-auto w-full">
          <thead className="text-left">
            <tr>
              <th className="border-black border-y-[1px] py-2">
                Tên sản phẩmx
              </th>
      ...
      <div className="ml-auto mb-5 w-[300px]">
        ...
        <div className=" mb-3 grid grid-cols-2">
          ...
          <div className="text-black-main">
            {saleInvoice?.order?.discountType === 1
              ? saleInvoice?.order?.discount + "%"
              : formatMoney(saleInvoice?.order?.discount)}
          </div>
        </div>
        ...
        <div className=" mb-3 grid grid-cols-2">
          ...
          <div className="text-black-main">
            {formatMoney(saleInvoice?.order?.cashOfCustomer)}
          </div>
        </div>
        <div className=" mb-3 grid grid-cols-2">
          ...
          <div className="text-black-main">
            {formatMoney(
              saleInvoice?.order?.cashOfCustomer -
                saleInvoice?.order?.totalPrice
            )}
          </div>
        </div>
      </div>
      <div className="text-center">
        {saleInvoice?.order ? (
          <p className="italic mt-4">
            (Bằng chữ:{" "}
            <span className="">
              {convertMoneyToString(+saleInvoice?.order?.totalPrice)} đồng)
            </span>
          </p>
        ) : (
          <p className="italic mt-4">
