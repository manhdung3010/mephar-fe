  ...
  return (
    <div >
      <div className='flex items-center flex-col'>
        ...
        <p className='font-bold'>Số HĐ: <span>{saleInvoice?.code}</span></p>
        <p>Ngày tạo: <span>{formatDateTime(saleInvoice?.createdAt)}</span></p>
      </div>
      <div className='mt-5'>
        <p>Khách hàng: <span className='ml-1'>{saleInvoice?.customer?.fullName}</span></p>
        <p>SĐT: <span className='ml-1'>{saleInvoice?.customer?.phone}</span></p>
        <p>Địa chỉ: <span className='ml-1'>{saleInvoice?.customer?.address}</span></p>
      </div>
      ...
      <div className="ml-auto mb-5 w-[300px]">
        ...
        <div className=" mb-3 grid grid-cols-2">
          ...
          <div className="text-black-main">{saleInvoice?.discountType === 1 ? saleInvoice?.discount + "%" : formatMoney(saleInvoice?.discount)}</div>
        </div>
