...
export function DiscountInfo({ record }: any) {
  ...
  const columns4: ColumnsType<IRecord> = [
    ...
    },
    {
      ...
      key: 'pointValue',
      className: 'max-w-[50%] w-1/2',
      render: (text, { discountType }) => <span>{formatNumber(text)}{discountType === "amount" ? "đ" : "%"}</span>,
    },
  ];
  const columns5: ColumnsType<IRecord> = [
    {
      ...
      key: 'productDiscount',
      className: '',
      render: (productDiscount) => <div className='flex gap-2 flex-wrap'>{
        // map item to product name from productDiscount
        productDiscount.filter((item) => item.isCondition).map((item) => {
          const product = products?.data?.items.find((product) => product.productUnit?.id === item.productUnitId);
          return product;
        }).map((product, index) => (
          <span className='bg-[#f0f0f0] rounded px-2 py-1' key={index}>{product?.productUnit?.code + " - " + product?.product?.name + " - " + product?.productUnit?.unitName}</span>
    ...
    },
    {
      ...
      key: 'discountValue',
      className: '',
      render: (discountValue, { discountType }) => <span>{formatNumber(discountValue)} {discountType === "amount" ? "đ" : "%"}</span>,
    },
    {
  ...
  const columns7: ColumnsType<IRecord> = [
    ...
    },
    {
      ...
      key: 'pointValue',
      className: '',
      render: (pointValue, { pointType }) => <span>{formatNumber(pointValue)}{pointType === "amount" ? "đ" : "%"}</span>,
    },
  ];
