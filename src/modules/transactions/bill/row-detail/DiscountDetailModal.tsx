...
import {
  EDiscountBillMethod,
  EDiscountGoodsMethod,
  EDiscountType,
} from "@/modules/settings/discount/add-discount/Info";
import { CustomModal } from "@/components/CustomModal";
import Image from "next/image";
import { getDiscountDetail } from "@/api/discount.service";
...
export function DiscountDetailModal({
  ...
  const { data: discountDetail, isLoading: isLoadingDiscount } = useQuery(
    ["DISCOUNT_DETAIL", discountId],
    () => getDiscountDetail(Number(discountId)),
    {
      enabled: !!discountId,
    },
  );
  ...
  return (
    ...
    >
      <CustomTable
        dataSource={discountDetail?.data?.data?.discountItem}
        columns={checkType(discountDetail?.data?.data?.target, discountDetail?.data?.data?.type)}
        bordered
        loading={isLoadingDiscount || isLoadingProduct}
        className="my-5"
      />
